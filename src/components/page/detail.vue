        ...
        <div class="detail_container">
            <div>
                <mt-navbar v-model="selected">
                    <mt-tab-item id="1">目标详情</mt-tab-item>
                ...
                <ul class="ul_detail ul_common_box">
                    <li>
                        <span class="name_left">目标描述</span>
                    ...
                    <template v-if="index === 0">
                      <li class="new_li" :key="index">
                        <span class="history_left">
                            ...
                            <span class="date">{{item.create_time.slice(item.create_time.indexOf('-')+1, item.create_time.indexOf(' '))}}</span>
                        </span>
                        <span class="history_right">
                            <div>
                                <span class="span_status">{{checkStatusName[item.audit_status]}}</span>:
...
export default {
  ...
  methods: {
    getDetail () {
      let self = this
      this.$ajax('post', this.HOST + '/tr/TargetAudit/web/getTargetDetail', {targetId: this.$route.params.id}, (res) => {
      if (res.success) {
    ...
    getHistory () {
      this.$ajax('post', this.HOST + '/tr/TargetAudit/web/getTargetAuditList', {targetId: this.$route.params.id}, (res) => {
        if (res.success) {
