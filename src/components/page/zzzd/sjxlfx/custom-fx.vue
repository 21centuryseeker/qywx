<template>
  <div class="content-fx lk-protect" >
    <div class="header">
      <table class="table-step">
        <tr>
          <td v-if="max>=1" :class="[ctab==1?'step_activite':'step_no_activite']" ><div @click="changeTap(1)"><span class="num_info">1</span>&nbsp;<span>选择指标</span></div></td>
          <td v-if="max>=2"  :class="[ctab==2?'step_activite':'step_no_activite']"><div @click="changeTap(2)"><span class="num_info">2</span>&nbsp;<span>选择时间</span></div></td>
          <td v-if="max>=3" :class="[ctab==3?'step_activite':'step_no_activite']"><div @click="changeTap(3)"><span class="num_info">3</span>&nbsp;<span>图表类型</span></div></td>
        </tr>
      </table>
    ...
    <div class="middle-content" :class="{'middle-content1':ctab==1}">
      ...
      <div v-show="ctab==2">
        ...
        <div class="timecss" v-show="shouCustom"  style="background-color: white">
          <div >
            <span>开始时间</span><input type="date"  v-model="startTime">
          </div>
          <div>
            <span>结束时间</span><input type="date" v-model="endTime" placeholder="asss">
          </div>
        </div>

      </div>
      <div v-show="ctab==3" style="font-size: 0.14rem;">
        ...
        <table width="100%" style="background-color: white;text-align: center">
          ...
          <tr @click="changeType('bar')">
            <td width="8%">
                    <span  style="display: inline-block">
                      <input type="radio" class="mint-radio-input" value="optionA">
                      <span class="mint-radio-core" :class="{'s_mint-radio-core  spec-core':'bar'==imgType}">
                      </span>
                    </span>
            </td>
            <td td width="18%">
    ...
    <div class="fx-fotter" style=" ">
      <table width="100%">
        <tr>
          <td>
            <mt-button v-if="ctab==2||ctab==max" size="large" @click="go(-1)" class="up_step">上一步</mt-button>
          </td>
          <td>
            <mt-button v-if="ctab==1||ctab==(max-1)" size="large" @click="go(1)">下一步</mt-button>
            <mt-button v-if="ctab==max " size="large" @click="back()">确定</mt-button>
          </td>
        </tr>
      </table>
...
<script>
  ...
  export default {
    ...
    mounted: function () {
      ...
      let {max=3,beforePath}  = this.$route.query;
      this.max = max;
      this.fromPath = beforePath;
      this.getJob().then(this.getTreeHtml);
    },
  }
...
<style >
  ...
  .lk-protect .middle-content { /* cjs */
    padding-top: 50px;
  }
  .lk-protect .middle-content1 { /* cjs */
    padding-top: 100px;
  }
  .lk-protect .middle-content tr:first-child  .cell-part-left{ /* cjs */
  ...
  .content-fx .spec-core::after{

  }
  .lk-protect .middle-content .middle-left {   /* cjs */
    height: 100vh;
